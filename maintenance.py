...
sql = open("core_schema.sql", "r").read()
cursor.execute(sql)
# cursor.execute(f'''SELECT id FROM client_cred WHERE email = %s ;''',['mad@dubakoor.com'])
# pl = cursor.fetchall()
# print(pl)
# print(len(pl))
# cursor.execute(f"""INSERT INTO user_cred (email, password, type) VALUES (%s,%s,%s);""",('ramu@dubakoor.com','Ramu@123','therapist'))
# cursor.execute(f"""INSERT INTO user_cred (email, password, type) VALUES (%s,%s,%s);""",('madhan@dubakoor.com','Madhan@123','client'))
